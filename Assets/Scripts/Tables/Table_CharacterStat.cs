...
public class Table_CharacterStat : Table_Base
{
    [Serializable]
    public class Info
    {
        public int m_nID;   // 아이디
        public int[] m_nStat = new int[(int)ePLAYERSTAT.ePLAYERSTAT_END];  // 스텟
        public int m_nSelect;   // 종류
    }
    ...
    protected bool _Read(CSVReader _reader,Info _info,int _nRow,int _nstartCol) // 테이블의 정보를 읽어오는 함수
    {
        ...
        _reader.get(_nRow, ref _info.m_nSelect);
