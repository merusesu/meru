...
public class CSVReader // CSV파일을 읽어오는 코드
{
    ...
    public void debug_output_grid() // 테이블 값을 디버그로 출력
    {
        ...
        for(int y = 0; y < _arr_grid.GetUpperBound(1); y++)
        {
            ...
            {
                textOutput += _arr_grid[x, y];  // 테이블의 행과 열
                textOutput += "|";              // 분기점을 나눈다
            }
            textOutput += "\n"; // 끝나면 다음줄로 넘어감
        }
        Debug.Log(textOutput);  // 받음값을 출력
    }

    public System.String[] get_row_array(int row)   // 한줄씩 읽어오는 코드
    {
    ...
    public bool is_data(int row,int col)    // 데이터 유무를 확인하는 코드
    {
    ...
    public int get_int(int row,int col) // int형 파싱(int형 데이터를 가져옴)
    {
        string s = _arr_grid[col, row];
        if ((s == null) || (s == ""))
            return 0;
    ...
    public bool reset_row(int row,int StartCol) // 한줄을 끝까지 읽으면 처음으로 되돌려서 읽기위한 코드
    {
        cur_col = StartCol; // 처음값을 따로 뺴주고
    ...
    public void get(int row,ref bool val)   // 행을 넣는 코드(해당하는 줄에 행들만 가져오는 코드,bool형)
    {
