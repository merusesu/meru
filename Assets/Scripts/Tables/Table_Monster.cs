...
public class Table_Monster : Table_Base
{
    ...
    public class Info
    {
        ...
        public int[] m_nStat = new int[(int)eMONSTERSTAT.eMONSTERSTAT_END];  // 스텟
        public int m_nMoney;   // 소지금
        public int m_nType; // 종류
        public string m_strFightIMG;  // 이미지
        public string m_strLobbyIMG;    // 로비 이미지
    ...
    protected bool _Read(CSVReader _reader,Info _info,int _nRow,int _nstartCol) // 테이블의 정보를 읽어오는 함수
    {
        ...
        _reader.get(_nRow, ref _info.m_nMoney);
